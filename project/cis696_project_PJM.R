...
## DATA PREP
data_prep <- defmacro(zonecode, expr = {
       # set the working directory
   # setwd("~/Dropbox/w2019/cis696/project/pjmdata")
   setwd("~/cis696/project/pjmdata")
   ...
   # zonecode: APS, DPL, or PJM 
   df.list <- subset_zone(blackout_rt, blackout_da, zonecode)
...
## NEURAL NET PREP
neural_net_prep <- defmacro(dataset, shuf, look, st, dl, bs, expr = {   
   ...
   # search_results.df <- hyper_param_search(dataset_list)
      ...
      # 60 %
   train_prop <- 0.70   # training proportion
   val_prop   <- 0.15   # validation proportion
   test_prop  <- 0.15   # testing proportion
...
# conv_units <- c(16, 32)
# lstm_units <- c(16, 32)
# lh <- hashmap(conv_units, lstm_units)
# 
# steps <- c(250, 500)
# epochs <- c(25, 50)
# se <- hashmap(steps, epochs)
# 
# mh.list = list()
# prelim()
# for (zone in zone_vec) {
#    data_prep(zone)
#    for (ds in dataset_vec) {
#       neural_net_prep(ds)
#       for (lkey in lh$keys()) {
#          for (skey in se$keys()) {
#             print(paste(zone, ds, lkey, lh[[lkey]], skey, se[[skey]]))
#             model_hist <- run_deep_model(lkey, lh[[lkey]], skey, se[[skey]])
#             mh.list <- list.append(mh.list, model_hist)
#          }
#       }
#    }
# }
